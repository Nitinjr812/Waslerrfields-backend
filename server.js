...
app.put('/api/products/:id', protect, upload.array('newImages', 5), async (req, res) => {
    try {
        ...
        // Make sure you have nodemailer/emailSender setup as per your config
        const { sendEmail } = require('./config/nodemailer');
        ...
        app.post('/api/payment/free-order', protect, async (req, res) => {
            try {
                ...
                // Check total must be 0 for free order
                ...
                for (const item of cart.items) {
                    ...
                    // Assume version is first one or match with item.version if saved
                    const version = product.versions.find(v => v.name === item.version) || product.versions[0];
                    if (!version || !version.r2MusicFile) continue;

                    const fileKey = version.r2MusicFile;
                    // Replace with your actual public bucket URL or URL generated by Cloudflare Worker
                    const fileUrl = `https://${process.env.R2_BUCKET_NAME}.r2.cloudflarestorage.com/${fileKey}`;
      ...
      <p>Your order has been processed successfully. Here are your download links (valid for 10 minutes):</p>
      <ul>
        ${downloadLinks.map(dl => `<li><strong>${dl.title}</strong> by ${dl.artist}: <a href="${dl.url}" target="_blank">Download</a></li>`).join("")}
      </ul>
      <p>Enjoy your music! <br/> The Waslerr Team</p>
    `;

                await sendEmail(user.email, "Your Waslerr Free Order - Download Links", emailHtml);

                return res.json({
                    success: true,
                    message: "Free order placed and download links sent to your email",
                    downloadLinks,
                    orderId: order._id
                });
